...
func (p *Picker) Build(buildInfo base.PickerBuildInfo) balancer.Picker {
	...
	for sc, scInfo := range buildInfo.ReadySCs {
		isLeaderValue := scInfo.Address.Attributes.Value("is_leader")
		isLeader := false
		if isLeaderValue != nil {
			isLeader = isLeaderValue.(bool)
		}
		if isLeader {
			p.leader = sc
			continue
...
func (p *Picker) nextFollower() balancer.SubConn {
	...
	followersLen := uint64(len(p.followers))
	idx := int(cur % followersLen)
